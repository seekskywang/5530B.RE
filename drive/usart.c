...
static void USART3_NVIC_Config(void)//Ԯࠚޓ˕א׏Ƥ׃
{
...
void USART3_Configuration()//ԮࠚԵʼۯگ˽
{  
    GPIO_InitTypeDef GPIO_InitStructure;
	USART_InitTypeDef USART_InitStructure;
...
//    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB,ENABLE); //ʹŜ GPIOA ʱד
...
//     GPIO_InitStructure.GPIO_OType = GPIO_OType_PP; //ΆάشԃˤԶ
	...
	USART_Cmd(USART3, ENABLE);//ʹŜԮࠚ1
    USART_ClearFlag(USART3, USART_FLAG_TC);
    

  
    USART3_NVIC_Config();//Ԯࠚא׏Ƥ׃
}
		...
		if (UART_Buffer_Send_pointer < Transmit_BUFFERsize)
         {
					 if(UART_Buffer_Send[0]==ADDR)
					 {
						...
						while (USART_GetFlagStatus(USART3,USART_FLAG_TXE) == RESET);//ֈսע̍Ϊԉ
						UART_Buffer_Send_pointer++;
					}
         }
		else
